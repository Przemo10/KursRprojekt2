...
test_that("throw.dice zwraca macierz",{
  expect_true(is.matrix(throw.dice(game, die1, die2)))
})

test_that("throw.dice nie dodaje psow",{
  expect_equal(unname(throw.dice(game, die1, die2))[c(2,5),4], c(0,0))
})

test_that("wolf.reaction zwraca macierz",{
  expect_true(is.matrix(wolf.reaction(game, "wolf")))
})

test_that("wolf.reaction usuwa kroliki",{
  game[1,4]<-2
  expect_equal(unname(wolf.reaction(game, "wolf"))[1,4], 0)
})

test_that("fox.reaction zwraca macierz",{
  expect_true(is.matrix(fox.reaction(game, "fox")))
})

test_that("fox.reaction usuwa kroliki",{
  game[1,4]<-2
  expect_equal(unname(fox.reaction(game, "fox"))[1,4], 0)
})
