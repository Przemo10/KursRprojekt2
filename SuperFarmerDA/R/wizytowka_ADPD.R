...
wizytowka_ADPD <- function( strategia,  N = 10000){
  ...
  wynik_strategii_max <-
    SuperFarmerDA::badaj_gre(SuperFarmerDA::strategia_maxrabbit , N)[[1]]
  wynik_strategii_badanej <-
    SuperFarmerDA::badaj_gre(strategia, N)[[1]]
  wynik_strategii_yolo <-
    SuperFarmerDA::badaj_gre(SuperFarmerRCNK::strategia_yolo, N)[[1]]
  
  strategia_max <- "maxrabbit"
  strategia_min <- "yolo"
  strategia_porownawcza <- deparse(substitute(strategia))
  strategia_porownawcza <-
    substr(
      strategia_porownawcza,
      nchar("strategia_") + regexpr("strategia_", strategia_porownawcza)[[1]],
      nchar(strategia_porownawcza)
    )
  
  
  
  czasy_gier <-
    data.frame(wynik_strategii_yolo,
               wynik_strategii_max,
               wynik_strategii_badanej)
  colnames(czasy_gier) <-
    c(strategia_min, strategia_max, strategia_porownawcza)
...
########### WYKRES PRZEDZIAŁY
  wykres_przedzialy <-
    ggplot2::ggplot(d2, ggplot2::aes(Var2, Freq, fill = Var1)) +
    ggplot2::geom_bar(position = "dodge", stat = "identity") +
    ggplot2::scale_y_continuous(expand = c(0, 0), labels = scales::percent) +
    ggplot2::theme(legend.position = "none") +
    ggplot2::labs(
      list(x = "Czas trwania gry (liczba rund)", y = "Procent w przedziale", title =
             "Porownanie wynikow strategii dla przedziałow czasu gry")
    ) +
    ggplot2::theme_bw() + ggplot2::theme(legend.position = "none")
  
  
######## UKŁAD NA STRONIE
  t1 <- gridExtra::ttheme_default(core = list(bg_params = list(
    fill = c(rep(c(
      "#F8766D", "#5CF058", "#619CFF"
    ))),
  ...
  wykres_tabela <-
    gridExtra::tableGrob(statystyki_strategii, rows = NULL, theme = t1)
  
  autorzy <-
    paste("Autorzy:", "Anna Dymowska" , "Przemyslaw Dycha", sep = "\n")
  autorzy <-
    grid::grid.text(
      autorzy,
  ...
  opis_slowny <-
    paste0(
      "Przedstawiamy statystyki strategii ",
      strategia_porownawcza ,
      " - strategii gry SuperFarmer dla jednego gracza z liczbą powtorzeń ",
      N,
      ". Dla porownania tej strategii na wykresach zamieściliśmy strategie: max_rabbit oraz yolo - najszybszą i najwolniejszą strategię znalezioną przez nas wśrod strategii, ktore przygotowali studenci na zajęcia z Programowania w R i wizualizacji danych w roku 2016/17."
    )
  
  opis_slowny <-
    RGraphics::splitTextGrob(opis_slowny, gp = grid::gpar(fontsize = 12))
  ...
  nazwa_pliku_pdf <-
    paste0("ADPD_wizytowka_", strategia_porownawcza, ".pdf")
