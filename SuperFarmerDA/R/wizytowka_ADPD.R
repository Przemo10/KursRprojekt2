...
wizytowka_ADPD <- function( strategia,  N = 10000){
  ...
  x <- SuperFarmerDA::badaj_gre( SuperFarmerDA::strategia_maxrabbit , N )[[1]]
  y <- SuperFarmerDA::badaj_gre( strategia, N )[[1]]
  z <- SuperFarmerDA::badaj_gre( SuperFarmerRCNK::strategia_yolo, N )[[1]]
  ...
  # Kolejnosc z,x,y jest istotna
  czasy_gier <- data.frame(z,x,y)
  colnames(czasy_gier) <- c(  strategia_min,strategia_max, strategia_porownawcza)


  ## Przeksztalcenie danych do tworzenia wykresu
