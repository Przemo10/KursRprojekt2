...
- `fox_leaves_rabbit`
- `wolf_eats_dog`  
- kostki `die1`, `die2`, których wartości są ustawione domyślnie jako `dice1` oraz `dice2` odpowiednio. Są one zawarte w pakiecie `SuperFarmerDA` i nie mogą być zastąpione przez inne kostki. Przykłady zastowań funckji `badaj_gre` dla dwóch strategii `strategia_maxrabbit` oraz `strategia_PDMS` z liczbą powtórzeń 1000. 
...
Funkcją wykonującą grę jest `gra`. Za argumenty przyjmuje ona `strategyname`, `die1` i `die2`. Zwraca liczbę rund wykonanych do zakończenia gry. Przykład :

```{r, results='asis'}
gra(strategia_maxrabbit,0,0,dice1,dice2)
```
...
Funkcją wykonującą ruch jest `make.move`. Przyjmuje ona za parametry stan obecny gry zawarty w tabeli `game` oraz nazwę strategii `strategia` i kostki `dice1`, `dice2`. Przykład użycia funkcji `make.move`:
...
`SuperFarmerDA::badaj_grw(SuperFarmer1::strategia_1,10,dice1,dice2)`
