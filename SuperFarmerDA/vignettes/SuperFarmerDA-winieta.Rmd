...
```{r }
 ...
 # ?examine.game
#examine.game(strategy_maxrabbit,1000,0,0,dice1,dice2)
examine.game(strategy_maxrabbit,10,0,0, dice1, dice2)
```
...
```{r, results='asis'}
play(strategy_maxrabbit,0,0,dice1,dice2)
```
...
Funkcją wykonującą ruch jest `make.move`. Przyjmuje ona za parametry stan obecny gry zawarty w tabeli `game` oraz nazwę strategii `strategy` i kostki `dice1`, `dice2`. Przykład użycia funkcji `make.move`:

```{r, results='asis'}
game[,"count"] <- c(10,1,2,0,0,0,0)
game <- make.move(game,strategy_maxrabbit,0,0,dice1,dice2)
knitr::kable(game)
```
...
`SuperFarmerDA::examine.game(SuperFarmer1::strategia_1,10,dice1,dice2)`
